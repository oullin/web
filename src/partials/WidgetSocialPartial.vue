<template>
	<div v-if="social.length > 0" class="rounded-lg border border-slate-200 dark:border-slate-800 dark:bg-linear-to-t dark:from-slate-800 dark:to-slate-800/30 p-5">
		...
		<div role="list" class="mt-5 space-y-2">
			<div v-for="item in social" :key="item.uuid">
				<div v-if="item.name === 'x'">
					<a target="_blank" rel="noopener noreferrer" :href="item.url" :title="item.description" class="group blog-widgets-social-links">
						...
						<span class="ml-4">Follow me on X</span>
					</a>
				</div>
				<div v-if="item.name === 'youtube'">
					<a target="_blank" rel="noopener noreferrer" :href="item.url" :title="item.description" class="group blog-widgets-social-links">
						...
						<span class="ml-4">Subscribe to my channel</span>
					</a>
				</div>
				<div v-if="item.name === 'instagram'">
					<a target="_blank" rel="noopener noreferrer" :href="item.url" :title="item.description" class="group blog-widgets-social-links">
						<svg viewBox="0 0 24 24" aria-hidden="true" class="blog-widgets-social-svg">
							<path d="M7.5,2h9A5.5,5.5,0,0,1,22,7.5v9A5.5,5.5,0,0,1,16.5,22h-9A5.5,5.5,0,0,1,2,16.5v-9A5.5,5.5,0,0,1,7.5,2ZM12,16A4,4,0,1,0,12,8,4,4,0,0,0,12,16Z" />
						</svg>
						<span class="ml-4">Follow me on instagram</span>
					</a>
				</div>
				...
				<div v-if="item.name === 'github'">
					<a target="_blank" rel="noopener noreferrer" :href="item.url" :title="item.description" class="group blog-widgets-social-links">
						...
						<span class="ml-4">Follow me on github</span>
					</a>
				</div>
			</div>
		</div>
